# QA Checklist — Task #22: Bank Reconciliation — statements, unreconciled txns, mark reconciled
...
### Read Handlers
- [ ] **CRITICAL** `bank.accounts.list` returns bank accounts from Sage
- [ ] **CRITICAL** `bank.transactions.unreconciled.list` returns unreconciled txns per account
- [ ] `bank.reconciliation.summary.get` returns summary (balance, uncleared count)
- [ ] Empty list returns empty array — not error

### Write Handler
- [ ] **CRITICAL** Respects WritesEnabled + tray consent gates
- [ ] **CRITICAL** `bank.transactions.reconcile.post` marks txn(s) reconciled in Sage
- [ ] Reconciled txn no longer appears in unreconciled list
- [ ] Idempotency key prevents duplicate reconcile

### Data Integrity
- [ ] Txns returned only from correct bank account (no cross-account bleed)

### Contract
- [ ] QueryResult + ExplainabilityEnvelope
- [ ] Handlers in HandlerCapabilityRegistry
