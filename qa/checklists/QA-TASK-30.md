# QA Checklist — Task #30: User Rights — role management UI + per-feature permission matrix
...
### Prerequisite
- [ ] **CRITICAL** Task #11 (RBAC v2 middleware) confirmed complete

### Role Management
- [ ] **CRITICAL** `GET /api/users` lists users with roles (admin only)
- [ ] **CRITICAL** `PUT /api/users/{id}/role` assigns role (admin only)
- [ ] Non-admin access → 403

### Permission Matrix
- [ ] **CRITICAL** Matrix covers: Inventory read, Inventory write, GL read, Report view, Order approve
- [ ] Each role has defined permissions
- [ ] Custom role creation with selected permissions works

### Enforcement
- [ ] **CRITICAL** Inventory write blocked for user without permission
- [ ] **CRITICAL** Order approve blocked for user without permission
- [ ] Admin bypasses all permission checks

### Immediacy
- [ ] Role change takes effect on next API request — no restart required
