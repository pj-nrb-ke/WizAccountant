# QA Checklist — Task #28: SaaS module — subscription plans, tenant onboarding, billing
...
### Plans
- [ ] **CRITICAL** Plans table: Free, Pro, Enterprise minimum
- [ ] Each plan has limits: max sites, max users, max jobs/day

### Onboarding
- [ ] **CRITICAL** `POST /api/auth/register` creates tenant + sends verification email
- [ ] Email verification link activates account
- [ ] New tenant defaults to Free plan

### Billing
- [ ] **CRITICAL** Flutterwave webhook upgrades plan on successful payment
- [ ] Upgrade takes effect immediately (no restart)
- [ ] Grace period logic correct

### Limits Enforcement
- [ ] **CRITICAL** Free tenant blocked on exceeding site limit
- [ ] **CRITICAL** Jobs/day counter resets at midnight EAT
- [ ] Over-limit returns clear message — not crash

### Security
- [ ] `FLUTTERWAVE_SECRET_KEY` from env var only — not hardcoded
