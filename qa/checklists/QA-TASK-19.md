# QA Checklist — Task #19: Billing hooks + compliance docs + app store pipeline
...
- [ ] **CRITICAL** `POST /api/billing/webhook` accepts Flutterwave payload
- [ ] **CRITICAL** Successful payment event upgrades tenant plan in DB
- [ ] Failed payment does NOT upgrade plan
- [ ] **CRITICAL** Webhook signature verified — invalid signature rejected
- [ ] Plan downgrade triggered on cancellation event

### Plan Enforcement
- [ ] Free plan: tenant cannot exceed Free site/user/job limits
- [ ] Pro plan limits applied correctly
- [ ] Grace period: expired plan allows X days before enforcement

### Compliance Docs
- [ ] Compliance documentation file exists under `/docs/compliance/`

### Security
- [ ] `FLUTTERWAVE_SECRET_KEY` from environment variable only — never hardcoded
