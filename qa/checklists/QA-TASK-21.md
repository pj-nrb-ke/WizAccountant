# QA Checklist — Task #21: Fixed Assets (minimal) — list, get, post depreciation
...
- [ ] **CRITICAL** NO asset creation endpoint exists
- [ ] **CRITICAL** NO asset update / edit endpoint exists
- [ ] **CRITICAL** NO advanced tracking (maintenance, insurance, disposal) endpoints exist
- [ ] Exactly THREE handlers exist: list, get, depreciation run

### List Assets
- [ ] **CRITICAL** `assets.list` returns asset list from Sage
- [ ] Response includes: code, description, book value, asset class

### Get Single Asset
- [ ] **CRITICAL** `assets.get` returns single asset with computed book value
- [ ] Book value = cost − accumulated depreciation (verify formula)
- [ ] Invalid code → clear error

### Depreciation Run
- [ ] **CRITICAL** Write handler respects WritesEnabled + tray consent gates
- [ ] **CRITICAL** `assets.depreciation.run.post` posts run via Sage SDK
- [ ] Book values updated in Sage
- [ ] Idempotency key prevents duplicate run

### Contract
- [ ] Registered in HandlerCapabilityRegistry
- [ ] QueryResult + ExplainabilityEnvelope returned
