# QA Checklist — Task #15: Multi-company connector support
...
### Company Selection
- [ ] **CRITICAL** Job request accepts optional `companyId` parameter
- [ ] Connector routes job to correct Sage company
- [ ] Invalid `companyId` returns clear error (not wrong-company query)

### Multi-company Routing
- [ ] **CRITICAL** Two simultaneous jobs for different companies return correct isolated data
- [ ] Company A data never appears in Company B response (verify with 2 test records)
- [ ] `GET /api/connector/companies?siteId=X` lists available companies

### Backwards Compatibility
- [ ] Job without `companyId` defaults to primary company — no breaking change
- [ ] Existing single-company deployments unaffected

### Write Safety
- [ ] Write job targets correct company only
