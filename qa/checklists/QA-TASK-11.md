# QA Checklist — Task #11: RBAC v2 — fine-grained role model + middleware
...
### Role Model
- [ ] **CRITICAL** Roles table in DB: Admin, Accountant, Viewer minimum
- [ ] User can be assigned a role via API
- [ ] Role persists after app restart

### Middleware
- [ ] **CRITICAL** No JWT → 401
- [ ] **CRITICAL** Valid JWT, wrong role → 403
- [ ] **CRITICAL** Valid JWT, correct role → 200
- [ ] Middleware applied on 3 random handler endpoints (spot-check)

### Admin Bypass
- [ ] Admin role passes all permission checks
- [ ] Non-admin cannot self-escalate role via API

### Existing Auth
- [ ] Login → JWT flow still works end-to-end
- [ ] Role visible in JWT claims payload
