# QA Checklist — Task #20: Stock Take — list sessions, post count adjustments
...
### Read Handlers
- [ ] **CRITICAL** `stocktake.sessions.list` returns active stock take sessions from Sage
- [ ] `stocktake.session.lines.get` returns count lines for a given session
- [ ] Empty session list returns empty array — not error

### Write Handler
- [ ] **CRITICAL** Write handler respects WritesEnabled + tray consent gates
- [ ] **CRITICAL** `stocktake.adjustment.post` posts finalised adjustments to Sage
- [ ] Inventory quantities updated in Sage after posting
- [ ] Idempotency key prevents duplicate adjustment

### Contract
- [ ] QueryResult rows returned
- [ ] ExplainabilityEnvelope included
- [ ] Handlers registered in HandlerCapabilityRegistry
