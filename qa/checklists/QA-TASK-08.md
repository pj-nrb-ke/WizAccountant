# QA Checklist — Task #8: Schema probe queries + connector metadata
...
- [ ] **CRITICAL** `GET /api/connector/metadata?siteId=test` returns HTTP 200
  _Paste response body (first 30 lines):_
- [ ] Response contains fields: `schemaVersion`, `tables`, `capabilities`
- [ ] Shape matches `ConnectorMetadataDto` contract

### Schema Probe Queries
- [ ] **CRITICAL** Handler returns available Sage tables for a connected site
- [ ] Handler returns empty list (not error) when connector is offline
- [ ] Second call within 60 s returns cached result (no re-query to Sage)

### Capability Registry
- [ ] New metadata entries visible via `GET /api/capabilities`
- [ ] No existing capability entries removed or broken
